About page. Prose only — see copy/home.md for the fragment format.

`record` is the apparatus table. Its rows are prose plus a link, so they live here
rather than in a data file: nothing else reads them and inventing a schema for six
rows would be structure for its own sake.

@@ heading
The practice

@@ intro
Ordoia is a {partyWord} AI assurance practice. We assess LLM and agent systems for grounding, reliability and production readiness before they are put in front of clients, auditors or regulators.
...
That places the practice alongside auditors rather than alongside development tools. It is also why the constraints matter more than the capability: nobody asks whether their tracing vendor is {partyWord}, because nothing turns on the answer.
...
@@ record.rows
- Method | Published in full. Current version OAL v{version}, dated {published}.
- Versioning | Changes classified as clarifying or breaking, with a one-line reason. Superseded versions stay published at permanent addresses, indefinitely. [Changelog](/changelog/)
- Retention | Working papers retained for six years, in redacted or derived form, with the redaction rule retained alongside.
- Licence | Creative Commons Attribution 4.0, with the level names reserved.
- Constraints | Published, and binding. [What {partyWord} means here](/independence/)
- Prices | Published, fixed before an engagement starts, never contingent on the score. [Services](/services/)
