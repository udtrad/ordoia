Independence page. Prose only — see copy/home.md for the fragment format.

`constraints` is a list in the order the handover fixed, which RATIONALE.md #8 records
as deliberate: the single-assessor disclosure sits fifth, not last, so that it does not
read as a retraction. Do not reorder it to suit a layout.

The section "The word we do not use" is deliberately literal and does not read the
terminology token. It is a discussion ABOUT the two words; substituting one for the
other would turn it into nonsense. See terminology.json.
...
@@ cannot.link
What the rubric itself cannot establish
